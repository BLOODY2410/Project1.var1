...
count_occurrences_substring PROC
    xor cx, cx                 ; Очищення лічильника для збереження кількості входжень
    mov bx, offset string      ; Встановлення покажчика на початок основної стрічки
    
outer_loop:     
    mov si, bx                 ; Встановлення покажчика SI на поточну позицію в основній стрічці
    mov di, offset substring   ; Встановлення покажчика DI на початок підстрічки
    mov dh, substring_length   ; Збереження довжини підстрічки в DH для подальших порівнянь

inner_loop:     
    mov al, [si]               ; Завантаження поточного символу основної стрічки
    mov ah, [di]               ; Завантаження поточного символу підстрічки
    cmp al, ah                 ; Порівняння символів основної стрічки та підстрічки
    jne not_matched            ; Якщо символи не співпадають, перехід до мітки not_matched
    inc si                     ; Перехід до наступного символу основної стрічки
    inc di                     ; Перехід до наступного символу підстрічки
    dec dh                     ; Зменшення лічильника залишкової довжини підстрічки
    jnz inner_loop             ; Якщо залишкова довжина не нульова, продовження перевірки
    
    inc bx                     ; Перехід до наступного символу в основній стрічці, якщо входження знайдено
    inc cl                     ; Інкрементування лічильника входжень

not_matched:                   
    inc bx                     ; Перехід до наступного символу в основній стрічці після невдалого співпадіння
    cmp byte ptr [bx], 0       ; Перевірка на кінець основної стрічки
    jnz outer_loop             ; Якщо кінець стрічки не досягнутий, продовження пошуку
    
    mov si, offset occurrences ; Встановлення покажчика на початок масиву входжень
    xor bx, bx
    mov bl, occurrences_length ; Завантаження поточної довжини масиву входжень
    shl bl, 1                  ; Подвоєння для отримання правильного зміщення (тому що кожен запис - 2 байти)
    add si, bx                 ; Додавання зміщення до покажчика на масив
    mov al, occurrences_length ; Повторне завантаження довжини для збереження в масив
    mov ah, cl                 ; Збереження кількості входжень у вищий байт
    mov [si], ax               ; Збереження пари "довжина:кількість входжень" у масив
    inc occurrences_length     ; Інкрементування довжини масиву входжень
    ret                        ; Повернення з процедури
